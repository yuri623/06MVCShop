...
<body bgcolor="#ffffff" text="#000000">
	...
	<%System.out.println("list.jsp의 order : "+((Search)request.getAttribute("search")).getOrder()); %>
		<form name="detailForm" action="/listProduct?menu=${param.menu}&order=${search.order}"
			method="post">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
					<td colspan="7">전체 ${resultPage.totalCount}건수, 현재
						...
						<a href="/listProduct?menu=${param.menu }&order=prod_name">상품명</a>
						&nbsp;<a href="/listProduct?menu=${param.menu }&order=manufacture_day">신상품순</a>
						&nbsp;<a href = "/listProduct?menu=${param.menu }&order=price">가격낮은순</a>
					...
					<tr class="ct_list_pop">
						...
						<td align="left"><c:if
							...
							<c:if test="${param.menu != 'search' || empty i.proTranCode }">
								<a href="/getProduct?prodNo=${i.prodNo}&menu=${param.menu}">${i.prodName}</a></td>
							</c:if>
						<td></td>
							...
							<c:if test="${user.role == 'admin' && !empty i.proTranCode}">
								<c:if test="${i.proTranCode == '1  '}">구매완료
									<c:if test="${param.menu == 'manage'}">
										<a href="/updateTranCodeByProd?prodNo=${i.prodNo}&tranCode=2">배송하기</a>
									</c:if>
							</c:if>
